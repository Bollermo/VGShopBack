...
function init(){
   ...
   /* if (currUser.admin == false){
    ...
    document.getElementById('usersBtn').addEventListener('click', e => {
        e.preventDefault();
        window.location.href = "http://localhost:8000/admin/users";
    });
    document.getElementById('reviewsBtn').addEventListener('click', e => {
        e.preventDefault();
        window.location.href = "http://localhost:8000/admin/reviews"
    });
    document.getElementById('gamesBtn').addEventListener('click', e => {
        e.preventDefault();
        window.location.href = "http://localhost:8000/admin/games";
    });
    document.getElementById('cartBtn').addEventListener('click', e => {
        e.preventDefault();
        window.location.href = "http://localhost:8000/admin/cart";
    });
    document.getElementById('wishlistBtn').addEventListener('click', e => {
        e.preventDefault();
        window.location.href = "http://localhost:8000/admin/wishlist";
    });
    document.getElementById('categoriesBtn').addEventListener('click', e => {
        e.preventDefault();
        window.location.href = "http://localhost:8000/admin/categories";
    });
    document.getElementById('publishersBtn').addEventListener('click', e => {
        e.preventDefault();
        window.location.href = "http://localhost:8000/admin/publishers";
    });
...
function getCurrUser() {
    // Pronađi kolačić prema imenu 'token'
    let token = document.cookie.split('; ').find(row => row.startsWith('token='));

    if (!token) {
        // Ako kolačić ne postoji, vrati null
        return null;
    }

    // Izvuci vrednost tokena
    token = token.split('=')[1];

    if (!token) {
        console.log("Prazan token");
        return null;
    }

    // Pokušaj da dešifruješ payload dela tokena
    try {
